...
class EventSummary(BaseModel):
    ...
    start_time: time = Field(
        ..., description='Time when the event starts in local time', example='22:38:19'
    )
    end_date: date = Field(..., description='Date when the event ends in local time')
    end_time: time = Field(
        ..., description='Time when the event ends in local time', example='14:45:15'
    )
